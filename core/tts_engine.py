...
def normalize_caps(text: str) -> str:
    ...
    def _convert_core(token: str) -> str:
        # Tamamen büyük harf değilse dokunma
        if not token.isupper():
            return token
        # Tek karakter 'I' → olduğu gibi bırak (İngilizce özne)
        if token == "I":
            return token
        # Bilinen gerçek kısaltmalar → olduğu gibi bırak
        if token in _KNOWN_ACRONYMS:
            return token
        # Diğer tüm ALL CAPS (2 karakter dahil) → Title Case
